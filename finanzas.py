...
def _detectar_columna_prioritaria(
    columnas: Sequence[str],
    candidatas: Sequence[str],
) -> Optional[str]:
    columnas_lista = list(columnas)
    columnas_casefold = {str(col).casefold(): col for col in columnas_lista}

    for candidata in candidatas:
        if candidata in columnas_lista:
            return candidata

        candidata_casefold = candidata.casefold()
        if candidata_casefold in columnas_casefold:
            return columnas_casefold[candidata_casefold]

    return None


def _inferir_delta_h(df_simulacion: pd.DataFrame, delta_h_default: float = 0.25) -> float:
    if "Fecha_Hora" not in df_simulacion.columns:
        return delta_h_default

    fechas = pd.to_datetime(df_simulacion["Fecha_Hora"], errors="coerce").dropna().sort_values()
    if len(fechas) < 2:
        return delta_h_default

    diferencias_h = fechas.diff().dropna().dt.total_seconds() / 3600.0
    diferencias_h = diferencias_h[diferencias_h > 0]
    if diferencias_h.empty:
        return delta_h_default

    delta_h = float(diferencias_h.median())
    if delta_h <= 0:
        return delta_h_default
    return delta_h


def _serie_potencia_no_negativa(
    df_simulacion: pd.DataFrame,
    columna: str,
    etiqueta: str,
) -> pd.Series:
    serie = pd.to_numeric(df_simulacion[columna], errors="coerce")
    if serie.notna().sum() == 0:
        raise ValueError(f"La columna de {etiqueta} '{columna}' no contiene valores numéricos.")
    return serie.fillna(0.0).clip(lower=0.0)
...
def calcular_finanzas_desde_simulacion(
    ...
    """
    Conecta la simulación solar con el resumen financiero Streger.

    El ahorro solar se calcula con energía autoconsumida, no con generación
    total. Para una serie de potencia:

        energia_autoconsumida_kwh = min(Demanda_kW, Generacion_Solar_kW) * delta_h

    Si no se puede inferir la resolución temporal desde Fecha_Hora, se usa
    delta_h = 0.25 h, consistente con datos quinceminutales.
    """
    if df_simulacion is None or df_simulacion.empty:
        raise ValueError("df_simulacion está vacío. Primero ejecuta la simulación solar.")

    columnas_demanda = ("Demanda_kW", "Demanda", "demanda_kw")
    columnas_generacion = (
        "Generacion_Solar_kW",
        "Generacion_kW",
        "Solar_kW",
    ...
    columnas_post_solar = ("Demanda_Post_Inyeccion_Solar_kW",)

    columna_demanda = _detectar_columna_prioritaria(df_simulacion.columns, columnas_demanda)
    columna_generacion = _detectar_columna_prioritaria(df_simulacion.columns, columnas_generacion)
    columna_post_solar = _detectar_columna_prioritaria(df_simulacion.columns, columnas_post_solar)

    faltantes = []
    if columna_demanda is None:
        faltantes.append(f"demanda ({', '.join(columnas_demanda)})")
    if columna_generacion is None:
        faltantes.append(f"generación solar ({', '.join(columnas_generacion)})")
    if faltantes:
        raise ValueError(
            "No se encontraron columnas suficientes para calcular el autoconsumo solar. "
            f"Faltan: {', '.join(faltantes)}."
        )

    delta_h_usado = float(delta_h) if delta_h is not None else _inferir_delta_h(df_simulacion)
    if delta_h_usado <= 0:
        raise ValueError("delta_h debe ser mayor a cero.")

    demanda_kw = _serie_potencia_no_negativa(df_simulacion, columna_demanda, "demanda")
    generacion_kw = _serie_potencia_no_negativa(df_simulacion, columna_generacion, "generación solar")

    energia_demanda_kwh = float(demanda_kw.sum() * delta_h_usado)
    energia_generada_kwh = float(generacion_kw.sum() * delta_h_usado)
    energia_autoconsumida_kwh = float(pd.concat([demanda_kw, generacion_kw], axis=1).min(axis=1).sum() * delta_h_usado)
    energia_excedente_kwh = float((generacion_kw - demanda_kw).clip(lower=0.0).sum() * delta_h_usado)

    energia_post_solar_kwh = None
    if columna_post_solar is not None:
        demanda_post_kw = _serie_potencia_no_negativa(
            df_simulacion,
            columna_post_solar,
            "demanda post solar",
        )
        energia_post_solar_kwh = float(demanda_post_kw.sum() * delta_h_usado)
    ...
    resumen = generar_resumen_financiero_streger(
        ...
        energia_autoconsumida_kwh_anual=energia_autoconsumida_kwh,
        inversion_fv_mxn=inversion_fv_mxn,
        inversion_bess_mxn=inversion_bess_mxn,
    )

    resumen["simulacion_solar"] = {
        "columna_demanda": columna_demanda,
        "columna_generacion_solar": columna_generacion,
        "columna_demanda_post_solar": columna_post_solar,
        "delta_h_usado": delta_h_usado,
        "energia_demanda_kwh_anual": energia_demanda_kwh,
        "energia_generada_solar_kwh_anual": energia_generada_kwh,
        "energia_autoconsumida_kwh_anual": energia_autoconsumida_kwh,
        "energia_excedente_kwh_anual": energia_excedente_kwh,
        "energia_post_solar_kwh_anual": energia_post_solar_kwh,
        "lectura": (
            "El ahorro FV usa solo energía autoconsumida. La generación excedente "
            "se reporta aparte y no se monetiza automáticamente como ahorro."
        ),
    }
